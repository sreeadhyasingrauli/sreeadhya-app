<<div class="p-6 bg-gray-100 min-h-screen">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-white p-4 rounded-lg shadow-sm border-l-4 border-red-500">
            ...
            <p class="text-2xl font-bold text-gray-800">${{ number_format($stats['overdue_amount'], 2) }}</p>
        </div>
        <div class="bg-white p-4 rounded-lg shadow-sm border-l-4 border-blue-500">
            <p class="text-sm text-gray-500 font-semibold uppercase">Invoices Today</p>
            <p class="text-2xl font-bold text-gray-800">{{ $stats['sales_today'] }}</p>
        </div>
    </div>
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody>
                ...
                <tr class="border-b hover:bg-gray-50">
                    <td class="py-3 font-semibold text-blue-600">{{ $invoice->invoice_number }}</td>
                    <td class="py-3">{{ $invoice->customer_id }}</td>
                    <td class="py-3">${{ number_format($invoice->invoice_amount, 2) }}</td>
                    <td class="py-3">
                        <span class="px-2 py-1 text-xs font-bold rounded {{ $invoice->status == 'paid' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
