    ...
    <div class="max-w-3xl mx-auto border p-8 rounded-lg shadow-sm">
        <div class="flex justify-between items-center mb-8">
            <div>
                ...
                <p class="text-sm text-gray-500">No: {{ $quote->quote_number }}</p>
            </div>
        ...
        <div class="mb-8">
            ...
            <p class="text-sm">{{ $quote->customer->name }}</p>
        ...
        <table class="w-full text-left mb-8 border-collapse">
            ...
            <tbody>
                ...
                <tr class="border-b border-gray-100 text-sm">
                    ...
                    <td class="py-4 px-2 text-right">${{ number_format($item->total_price, 2) }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
        ...
        <div class="flex justify-end">
            <div class="w-64 text-sm space-y-2">
                <div class="flex justify-between"><span>Subtotal:</span><span>${{ number_format($quote->subtotal, 2) }}</span></div>
                <div class="flex justify-between"><span>Tax (15%):</span><span>${{ number_format($quote->tax_amount, 2) }}</span></div>
                <div class="flex justify-between font-bold text-lg border-t pt-2 text-indigo-600">
                    <span>Total:</span><span>${{ number_format($quote->total, 2) }}</span>
                </div>
            </div>
        </div>
    </div>
