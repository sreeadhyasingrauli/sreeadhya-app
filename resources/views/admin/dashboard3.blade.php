...
<div class="p-6 max-w-7xl mx-auto space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            ...
            <p class="text-2xl font-bold text-amber-600">{{ number_format($stats['pending_amount'], 2) }}</p>
        </div>

        <!-- Count Indicator Card -->
        <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            <p class="text-sm font-medium text-gray-500">Invoices Generated Today</p>
            <p class="text-2xl font-bold text-gray-900">{{ $stats['today_bills'] }}</p>
        </div>
    </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-gray-100 text-sm">
                        ...
                        <tr>
                            <td class="py-3 font-medium text-gray-900">{{ $invoice->invoice_number }}</td>
                            <td class="py-3 text-gray-600">{{ $invoice->customer_id }}</td>
                            <td class="py-3 text-gray-900">${{ number_format($invoice-invoice_amount, 2) }}</td>
                            <td class="py-3">
