...
<div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
    ...
    <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-100">
        ...
        <p class="text-2xl font-bold text-amber-500">${{ number_format($pendingAmount, 2) }}</p>
    </div>

    <!-- Paid Invoices Count -->
    <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-100">
        <p class="text-sm font-medium text-gray-500 uppercase">Paid Invoices</p>
        <p class="text-2xl font-bold text-gray-800">{{ $paidCount }}</p>
    </div>
    ...
    <table class="w-full text-left border-collapse">
        ...
        <tbody class="text-gray-600 text-sm">
            ...
            <tr class="border-b border-gray-100 hover:bg-gray-50">
                <td class="py-3 font-medium text-blue-600">#{{ $invoice->invoice_number }}</td>
                <td class="py-3">{{ $invoice->user->name }}</td>
                <td class="py-3">${{ number_format($invoice->total, 2) }}</td>
                <td class="py-3">
                ...
                <td class="py-3">{{ $invoice->created_at->format('M d, Y') }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
